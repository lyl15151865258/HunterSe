...
/**
 * 摇杆自定义控件
 * Created at 2022-06-14 0014 15:26:42
 *
 * @author LiYuliang
 * @version 1.0
 */
public class RockerView extends View {
    ...
    private static final float DEFAULT_ROCKER_SCALE = 0.1f;//默认半径为背景的1/2
...
//        mAreaRadius = (measuredWidth <= measuredHeight) ? (int) (cx / (mRockerScale + 1)) : (int) (cy / (mRockerScale + 1));
    ...
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        Log.e("摇杆实际显示", "lenXY:" + lenXY + "***regionRadius:" + mAreaRadius + "***距离占比:" + lenXY / mAreaRadius);
        if (lenXY + rockerRadius <= regionRadius) { // 触摸位置在可活动范围内
            // 回调 返回参数
            callBack(angle, lenX / (mAreaRadius - rockerRadius), lenY / (mAreaRadius - rockerRadius), lenXY / (mAreaRadius - rockerRadius));
            return touchPoint;
        } else { // 触摸位置在可活动范围以外
            // 计算要显示的位置
...
//            callBack(angle, (int) Math.sqrt((showPointX - centerPoint.x) * (showPointX - centerPoint.x) + (showPointY - centerPoint.y) * (showPointY - centerPoint.y)));
            callBack(angle, (float) ((regionRadius - rockerRadius) * Math.cos(radian) / (mAreaRadius - rockerRadius)),
                    (float) ((regionRadius - rockerRadius) * Math.sin(radian)) / (mAreaRadius - rockerRadius), 1);
     ...
     * @param angle 摇动角度
     */
    private void callBack(double angle, float x, float y, float distance) {
        Log.e("距离圆心点间距distance", distance + "");
        if (null != mOnDistanceLevelListener) {
            mOnDistanceLevelListener.onDistanceLevel(x, y, distance);
        }
...
//        }
    ...
    public interface OnDistanceLevelListener {
        ...
        void onDistanceLevel(float x, float y, float level);
    }
